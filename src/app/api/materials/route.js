...
import { getUserFromCookie, getUserWalletAddress } from "@/lib/auth";
...
export async function GET(request) {
  try {
    ...
    // Return demo data for "Top Shared Study Materials"
    if (isTopShared) {
      ...
      return NextResponse.json({
        featured: featuredMaterial,
        items: topSharedMaterials,
        total: topSharedMaterials.length,
        isTopShared: true
      });
    }
    ...
    // Return demo data for "Discover More Study Materials"
    if (isDemo) {
      const demoMaterials = [
        {
          id: "demo-1",
    ...
    // Original authenticated user materials endpoint
    const user = await getUserFromCookie(request);
    if (!user) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
